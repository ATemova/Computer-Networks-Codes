...
*	Note: Appnum is optional. If not specified the standard echo appnum (7) is used.
*-------------------------------------------------------------------------------------
*/
int int main(int argc, char const *argv[])
{
	...
	if (argc == 3)
		...
		while((len = readln(buff, BUFFSIZE)) > 0) {
			...
			/* read and print same number of bytes from echo server */
			expect = len;
			for (received = 0; received < expect;) {
				len = recv(conn, buff, (expect - received) < BUFFSIZE ? (expect - received) : BUFFSIZE, 0);
				if (len < 0) {
					send_eof(conn);
					return 1;
				}
				(void) write(STDOUT_FILNEO, buff, len);
				received += len;
			}
			(void) printf("\n");
			(void) printf(INPUT_PROMPT);
